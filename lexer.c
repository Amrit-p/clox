...
Token lexer_parse_id(Lexer *lexer)
{
    ...
    case 'c':
        ...
        {
        case 'o':
            switch (lexer->src[start + 2])
            {
            case 'n':
                switch (lexer->src[start + 3])
                {
                case 't':
                    token_type = lexer_check_keyword(lexer, 4, "inue", TOKEN_CONTINUE);
                    break;
                case 's':
                    token_type = lexer_check_keyword(lexer, 1, "t", TOKEN_CONST);
                    break;
                default:
                    break;
                }
    ...
    case 'f':
        ...
        break;
    case 'b':
        token_type = lexer_check_keyword(lexer, 4, "reak", TOKEN_BREAK);
        break;
    default:
        break;
    }
...
Token lexer_parse_string(Lexer *lexer)
{
    ...
    char start_char = lexer->current_char;
    lexer_advance(lexer);
    size_t start = lexer->index;
    while (lexer->current_char != start_char)
    {
        lexer_advance(lexer);
        if (lexer->current_char == '\n' || lexer->current_char == '\0')
            break;
    }
    if (lexer->current_char == start_char)
    {
